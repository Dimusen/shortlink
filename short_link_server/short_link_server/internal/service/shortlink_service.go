...
func NewShortLinkService(repo repository.ShortLinkRepository, redisClient *redis.Client, baseURL string) ShortLinkService {
	...
	bloomFilter := utils.NewBloomFilter(redisClient, "shortlink:bloomfilter", 1000000, 0.01)
	return &shortLinkServiceImpl{repo: repo, redisClient: redisClient, baseURL: baseURL, bloomFilter: bloomFilter}
}
...
func (s *shortLinkServiceImpl) CreateShortLink(originalURL string, expiresAt string) (*model.CreateShortLinkResponse, error) {
	...
	for i := 0; i < 10; i++ { // 最多尝试10次
		...
		// 使用布隆过滤器快速检查短码是否可能存在
		exists, _ := s.bloomFilter.Contains(shortCode)
		if !exists {
			// 如果布隆过滤器说不存在，那肯定不存在，可以直接使用
			break
		}
		// 布隆过滤器说可能存在，需要去数据库确认
...
func (s *shortLinkServiceImpl) GetOriginalURL(shortCode string) (string, error) {
	// 1. 先使用布隆过滤器快速检查短码是否可能存在
	exists, err := s.bloomFilter.Contains(shortCode)
	if err != nil {
		// 布隆过滤器检查失败，继续执行原有逻辑
	} else if !exists {
		// 如果布隆过滤器说不存在，那肯定不存在，直接返回错误
